...
if (!featureFlag) {
    throw Error(`No feature toggle name found. Specify the toggle name using 'TAOTIE_FEATURE_TOGGLE_NAME' env variable.`);
}
...
const featureFlagIdentifier = ({__a}, path) => {
    ...
    if (__a.type === 'Identifier' && __a.name !== featureFlag)
        return;
